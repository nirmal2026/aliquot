...
#about p{font-size:12.5px;line-height:1.5;color:var(--sub);margin:.5em 0}
...
<div id="about">
  <div class="card2">
    ...
    <h2>Envicron</h2>
    <p><b>Offline environmental laboratory &amp; field calculators.</b> 91 routines across water,
    air, source emission, soil, waste, noise and QA/QC — each with the standard, method number and
    clause it rests on. Nothing you enter leaves this device; every calculation runs offline.</p>
    <p><b>Author:</b> Dr. Nirmal Kumar Sharma. A personal project.</p>
    <div class="disc">
    <b>Not an official product.</b> Envicron is not a product of the Central Pollution Control Board,
    the Ministry of Environment, Forest &amp; Climate Change, or the Bureau of Indian Standards, and
    no endorsement by any of them is claimed or implied.<br><br>
    <b>A calculation aid, not a validated method.</b> Under ISO/IEC 17025:2017 &sect;7.11.2, verify
    each routine against the current edition of the standard cited before relying on any result.
    Regulatory limits change — confirm against the notification in force. This app contains no
    internal SOP code, format or unpublished material.
    </div>
    <p style="margin-top:10px">Version 1.0 &middot; built from the merged source tree.</p>
  </div>
</div>
<div id="gate">
  <div class="box">
    ...
    function buildBody(){
      ...
      rec.addEventListener("click", function(){
        ...
        pdf.addEventListener("click", function(){
          ...
          // Build a fully standalone printable HTML document and hand it to the
          // system as a download. In a content:// WebView window.print() and the
          // Web Share API are unavailable, so a Blob download is the reliable path:
          // the user then opens it and uses the OS "Print > Save as PDF".
          var doc = "<!doctype html><html><head><meta charset=utf-8>"
            + "<meta name=viewport content=\'width=device-width,initial-scale=1\'>"
            + "<title>" + esc(fn.value || rt.name) + "</title><style>"
            + "body{font-family:Georgia,serif;color:#000;background:#fff;padding:16px;font-size:12px}"
            + "h2{font-size:16px;margin:0 0 4px} .meta{font-size:11px;color:#333;margin-bottom:10px}"
            + "table{width:100%;border-collapse:collapse;font-size:11px} "
            + "th,td{border:1px solid #ccc;padding:4px 6px;text-align:left;vertical-align:top} "
            + "th{background:#f0f0f0} .sig{margin-top:24px;font-size:11px}"
            + "</style></head><body>" + recHtml + "</body></html>";
          var ok = downloadFile((fn.value||rt.id) + ".html", doc, "text/html");
          // also try native print for browsers that support it
          try {
          ...
          if(ok){ pdf.textContent = "Saved ✓"; setTimeout(function(){ pdf.textContent="Save as PDF"; }, 1600); }
        });
        shr.addEventListener("click", function(){
          persist();
          var txt = reportText(rt, lr, fn.value, an.value, lb.value);
          // 1) native share if truly available
          if(navigator.share){
            navigator.share({title: fn.value || rt.name, text: txt}).catch(function(){});
            return;
          }
          // 2) clipboard API if available
          if(navigator.clipboard && navigator.clipboard.writeText){
            navigator.clipboard.writeText(txt).then(function(){
              shr.textContent = "Copied ✓"; setTimeout(function(){ shr.textContent="Share"; }, 1600);
            }, function(){ legacyCopy(txt, shr); });
            return;
          }
          // 3) legacy execCommand copy (works in content:// WebView)
          legacyCopy(txt, shr);
        });
      });
    }
