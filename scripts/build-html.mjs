...
(function(){
  ...
  function saveReport(name, htmlDoc, txt, btn){
    if(isNative()){
      ...
      if(!Fs){ showText(txt, "Could not reach the file system on this build. The report text is below \u2014 long-press to copy."); return; }
      // Write to CACHE: the Share plugin's built-in FileProvider
      // (the app fileprovider) covers the cache folder by default, so the
      // file:// path from getUri can be shared as a safe content:// uri with no
      // extra native config. Then hand it to the Android share/open sheet, where
      // the user picks "Print > Save as PDF", Drive, Gmail, etc.
      var fpath = name + ".html";
        ...
        .then(function(u){
          ...
          if(Sh && fileUrl){
            return Sh.share({ title: name, files: [fileUrl], dialogTitle: "Save or share report" })
              .then(function(){ flash(btn,"Done \u2713","Save as PDF"); });
          }
          // no Share plugin: at least confirm the write and show text
          flash(btn,"Saved \u2713","Save as PDF");
          showText(txt, "Saved to app storage. Sharing is unavailable on this build; report text below.");
        })
        .catch(function(e){
          showText(txt, "Save failed: " + (e && (e.message||e.errorMessage) || e) + ". The report text is below \u2014 long-press to copy.");
        });
      return;
    }
    // Browser (desktop preview): real download + print dialog.
    ...
    showText(txt, "Download is not available here. Report text below.");
  }
