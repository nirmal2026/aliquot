...
<div id="about">
  <div class="card2">
    ...
    project. It is not issued, endorsed or approved by any government department, regulatory body
    or standards organisation, and no such endorsement is claimed or implied. It contains no
    internal procedure code, official format or unpublished material.</p>
...
<header>
  ...
  <div class="sub">Offline environmental laboratory &amp; field calculators · <span id="count"></span></div>
...
(function(){
  ...
  /* ---- Capacitor-aware save & share ------------------------------------
     ...
     fails. ---------------------------------------------------------------- */
  function cap(){ return (window.Capacitor && window.Capacitor.Plugins) ? window.Capacitor.Plugins : null; }
  function flash(btn,msg,back){ if(!btn) return; var o=btn.textContent; btn.textContent=msg;
    setTimeout(function(){ btn.textContent=back||o; }, 1700); }

  function showText(txt){
    // last-resort visible, selectable copy of the report
    var m=document.getElementById("about");
    var pre=document.createElement("div"); pre.className="savepanel";
    pre.style.whiteSpace="pre-wrap"; pre.style.userSelect="text"; pre.textContent=txt;
    var host=document.querySelector(".savepanel"); if(host&&host.parentNode) host.parentNode.appendChild(pre);
  }

  function saveReport(name, htmlDoc, txt, btn){
    var P=cap();
    if(P && P.Filesystem){
      // write .html into Documents, then offer to open/share it
      P.Filesystem.writeFile({
        path: name + ".html", data: htmlDoc, directory: "CACHE", encoding: "utf8", recursive:true
      }).then(function(res){
        flash(btn,"Saved \u2713","Save as PDF");
        // try to open the saved file so the user can Print > Save as PDF
        if(P.FileOpener && res && res.uri){
          P.FileOpener.open({ filePath: res.uri, contentType: "text/html" }).catch(function(){});
        } else if(P.Share && res && res.uri){
          P.Share.share({ title:name, url:res.uri }).catch(function(){});
        }
      }).catch(function(e){
        // filesystem denied -> share the text instead
        shareReport(name, txt, btn);
      });
      return;
    }
    // browser: blob download
    if(downloadFile(name + ".html", htmlDoc, "text/html")){ flash(btn,"Saved \u2713","Save as PDF");
  ...
  function shareReport(name, txt, btn){
    var P=cap();
    if(P && P.Share){
      P.Share.share({ title:name, text:txt, dialogTitle:"Share report" })
        .then(function(){}).catch(function(){ copyText(txt, btn); });
      return;
    }
    ...
    function buildBody(){
      ...
      rec.addEventListener("click", function(){
        ...
        pdf.addEventListener("click", function(){
          ...
          saveReport((fn.value||rt.id), htmlDoc, txt, pdf);
        });
