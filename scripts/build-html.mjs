...
(function(){
  ...
  function emailReport(name, txt, btn, say){
    say = say || function(){};
    var to = "nir01471@gmail.com";
    var subject = "Envicron report — " + name;
    // Keep the body within a safe length; mail apps vary. Trim with a note.
    var body = txt;
    var MAX = 1800;
    if(body.length > MAX){ body = body.slice(0, MAX) + "\\n\\n[Report truncated for email. Use Save as PDF for the full record.]"; }
    var href = "mailto:" + encodeURIComponent(to)
             + "?subject=" + encodeURIComponent(subject)
             + "&body=" + encodeURIComponent(body);
    try{
      // Prefer Capacitor App/Browser openers if present, else location.
      window.location.href = href;
      say("Opening your email app to " + to + "\u2026 tap Send. (Attachment is not possible by email; use Save as PDF for the file.)");
      flash(btn,"Opening\u2026","Email report");
    }catch(e){
      say("Could not open the email app: " + e + "\\nThe report text is below \u2014 long-press to copy.", true);
      showText(txt);
    }
  }
  ...
  function card(rt){
    ...
    function buildBody(){
      ...
      rec.addEventListener("click", function(){
        ...
        var eml = el("button","sec","Email report");
        srow.appendChild(pdf); srow.appendChild(shr); srow.appendChild(eml);
        sp.appendChild(srow);
        ...
        pdf.addEventListener("click", function(){
          ...
          saveReport((fn.value||rt.id), htmlDoc, txt, pdf, setStatus);
        });
        ...
        eml.addEventListener("click", function(){
          persist();
          var txt = reportText(rt, lr, fn.value, an.value, lb.value);
          emailReport((fn.value||rt.name), txt, eml, setStatus);
        });
      });
    }
    return c;
